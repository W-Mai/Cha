...
fn extract_function(
    ...
    Some(FunctionInfo {
        ...
        switch_arm_values: body
            .map(|b| collect_py_arm_values(b, src))
            .unwrap_or_default(),
        external_refs: body
...
fn collect_py_arm_values(body: Node, src: &[u8]) -> Vec<cha_core::ArmValue> {
    let mut out = Vec::new();
    crate::switch_arms::walk_arms(body, src, &mut out, &|n| n.kind() == "case_clause");
    out
}
